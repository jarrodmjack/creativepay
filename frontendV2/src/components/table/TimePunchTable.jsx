...
const TimePunchTable = ({ timePunches }) => {
	...
	const currentTimepunches = tableTimePunches.slice(
		indexOfFirstTimepunch,
		indexOfLastTimePunch
	)
	...
	const handleSubmitTimePunch = async (timePunch) => {
		try {
			...
			window.alert(
				"There was an issue submitting the timepunch. Please ensure the information entered is correct and try again"
			)
		}
	}
	...
	const handleFilterTimePunches = (fromDate, toDate, employeeName) => {
		fromDate = new Date(fromDate)
		toDate = new Date(toDate)
		if (!employeeName) {
			setFilteredTimePunches(
				tableTimePunches.filter((timepunch) => {
			...
			setFilteredTimePunches(
				tableTimePunches.filter((timepunch) => {
					let timepunchDate = new Date(timepunch.date)
					if (timepunchDate <= toDate && timepunchDate >= fromDate && timepunch.employeeName === employeeName) {
						return timepunch
					}
				})
			)
	...
	const handleDeleteTimePunch = async (id) => {
		try {
			...
			if (response.ok) {
				setTableTimePunches(
					currentTimepunches.filter(
						(timePunch) => timePunch._id !== id
					)
				)
			} else {
	...
	return (
		<div>
			<div className="overflow-x-auto shadow-xl">
				<TimePunchCreationForm
					handleSubmitTimePunch={handleSubmitTimePunch}
				/>
				<TableOptions
					handlePrint={handlePrint}
					handleFilterTimePunches={handleFilterTimePunches}
					handleResetFilter={resetTimePunchFilter}
				/>
			...
			{!filteredTimePunches.length && (
				<Pagination
					timepunchesPerPage={timepunchesPerPage}
